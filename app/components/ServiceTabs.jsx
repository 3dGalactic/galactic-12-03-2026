...
const services = [
  {
    id: "prototyping",
    title: "Rapid Prototyping",
    description: "Test and refine your designs quickly with high-speed prototyping services.",
    list: ["Concept validation in days", "Functional testing capabilities", "Certified materials available"],
    images: ["/services/rp.jpg", "/services/rp2.jpg", "/services/rp3.jpg"],
  },
  {
    id: "production",
    title: "Full-Scale Production",
    description: "From low-volume batches to large-scale runs with precision and reliability.",
  ...
  },
  {
    id: "custom",
    title: "Custom Manufacturing",
    description: "Tailored solutions for complex geometries and specialized industrial applications.",
...
export default function ServiceTabs() {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [activeIndex, active.images.length]);
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <div className="relative rounded-2xl border border-white/10 bg-white/[0.05] backdrop-blur-xl overflow-hidden">
        ...
        <div className="block   lg:hidden p-6">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="mb-8"
            >
              <h3 className="text-2xl font-['dena'] mb-3 text-white text-center">{active.title}</h3>
              <p className="text-gray-400 font-['scrib'] text-sm text-center mb-4">{active.description}</p>
              <ul className="flex flex-wrap justify-center gap-x-4 gap-y-1">
                {active.list.map((item, i) => (
                  <li key={i} className="text-[10px] uppercase tracking-wider text-gray-500 flex items-center gap-1">
          ...
          {/* 2. Images in the middle (Above the titles) */}
          <div className="relative h-[240px] mr-[15vw]   mb-[30vw]">
            {images.map((img, i) => {
              const pos = positions[i];
              return (
                <motion.img
                  ...
                  src={img}
                  layout
                  className="absolute w-[95%]   aspect-[4/3] rounded-xl object-cover border border-white/10 shadow-2xl"
                  style={{
                    top: pos.top,
                    left: pos.left,
                    transform: `rotate(${pos.rotate}deg)`,
          ...
          {/* 3. Titles stacked one below the other */}
          <div className="flex relative  flex-col gap-3">
            {services.map((service, i) => (
              <button
                key={service.id}
                onClick={() => setActiveIndex(i)}
                className={`w-full py-4 px-6 rounded-xl text-left transition-all duration-300 border
                ${activeIndex === i 
                  ? "bg-white/10 border-white/20 text-white translate-x-2" 
                  : "bg-white/5 border-transparent text-gray-500"}`}
              >
                <div className="text-[10px] opacity-40 uppercase tracking-[0.2em] mb-1">0{i + 1}</div>
                <div className="font-['dena'] text-lg">{service.title}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="hidden lg:grid lg:grid-cols-[320px_1fr]">
          <div className="border-r border-white/10 p-6">
            <div className="flex flex-col gap-2">
              {services.map((service, i) => (
                <div
                  ...
                  onMouseEnter={() => setActiveIndex(i)}
                  className={`cursor-pointer px-4 py-4 rounded-lg transition
                  ${activeIndex === i ? "bg-white/10 text-white" : "text-gray-400 hover:bg-white/5 hover:text-gray-200"}`}
                >
                  <div className="text-sm opacity-60 mb-1">{service.id}</div>
                  <div className="font-['dena']">{service.title}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-10 grid md:grid-cols-2 gap-10 items-center min-h-[460px]">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ duration: 0.35 }}
              >
                <h3 className="text-2xl font-['dena'] mb-4">{active.title}</h3>
                <p className="text-gray-300 font-['scrib'] mb-6">{active.description}</p>
                <ul className="space-y-2">
                  {active.list.map((item, i) => (
                    <li key={i} className="flex gap-3 text-gray-300">
                      <span className="mt-[7px] w-[6px] h-[6px] rounded-full bg-white/70" />
